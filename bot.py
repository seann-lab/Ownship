...
def _ip_check_one(proxy_url: str, timeout: int = 15, settings: dict = None) -> dict:
    """Melakukan eksekusi validasi IP melalui proxy_url dengan multiple endpoints."""
    try:
        import socks
    except ImportError:
        return {"error": "SOCKS support belum terpasang."}

    sess = http_requests.Session()
    proxies = {"http": proxy_url, "https": proxy_url}
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/125.0.0.0 Safari/537.36"}

    # List endpoint untuk menghindari rate limit
    endpoints = [
        "http://ip-api.com/json/?fields=status,countryCode,query,city,isp",
    ...
    for url in endpoints:
        try:
            r = sess.get(url, proxies=proxies, timeout=timeout, headers=headers)
            if r.status_code == 200:
                data = r.json()
                ip = data.get("query") or data.get("ip") or data.get("origin") or data.get("ip_addr")
                if ip:
                    # Jika menggunakan ip-api, kita bisa cek negara langsung
                    if "ip-api.com" in url:
                        if data.get("countryCode") != "BR":
                            continue # Cari IP Brazil saja
...
def _ip_scan_sync(settings: dict, target: int = 3, max_attempts: int = 150, min_score: int = 70, timeout: int = 10):
    import queue
    ...
    def _worker_task(attempt_num):
        try:
            # Beri sedikit delay antar thread agar tidak menumpuk di awal
    ...
    # Tambah jumlah workers agar lebih agresif mencari IP
    max_workers = 100
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for i in range(1, max_attempts + 1):
            executor.submit(_worker_task, i)

        start_time = time.time()
        # Beri waktu lebih lama jika mencari banyak IP
        max_wait = 180
        
        while len(clean_ips) < target and (time.time() - start_time) < max_wait:
            try:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "menu_home":
            ...
            loop = asyncio.get_running_loop()
            clean_ips, _, _ = await loop.run_in_executor(None, _ip_scan_sync, s, target_count, 200, 70, 15)
            ...
            # Pembuatan list proxy rotator secara langsung (tanpa parsing card)
            # Selalu gunakan socks5 untuk rotator sesuai permintaan user
            scheme = "socks5"
            port = 1080
            ...
            # Mendapatkan BOT_TOKEN dan CHAT_ID untuk disisipkan ke file rotator
            bot_token = s.get("bot_token") or os.environ.get("BOT_TOKEN", "")
            chat_id = str(query.message.chat_id)
            
            # Template lengkap proxy rotator sesuai contoh user
    ...
    while True:
        ...
        with lock:
            if PROXIES:
                ...
                # Dynamic sess extraction for FlameProxies
...
"""
            ...
            with open(file_name, "rb") as f:
                await context.bot.send_document(
                    ...
                    document=f,
                    filename=file_name,
                    caption=f"✅ **Ditemukan {len(clean_ips)} IP Clean!**\n\nFile proxy rotator telah dibuat dengan format SOCKS5 port 1080.",
                    parse_mode="Markdown"
                )
            
            # Hapus pesan loading dan file sementara
