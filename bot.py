...
def _ip_scan_sync(settings: dict, target: int = 3, max_attempts: int = 30, min_score: int = 70, timeout: int = 8):
    ...
    # Instruksi 6: IP dari probe awal langsung di-append ke clean_ips tanpa diuji ulang
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "menu_home":
            ...
            # Instruksi 5: Ekstraksi dinamis berdasarkan settings.get("proxy_host")
            target_host = s.get("proxy_host", "proxy.flameproxies.com")
            for ip_data in clean_ips:
                card = _format_ip_card(ip_data, 1, settings=s)
                for line in card.split("\n"):
                    if "@" in line and target_host in line:
