...
async def cmd_apidebug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        r = sms_session.get(f"{SMSCODE_BASE}/catalog/services", headers=sms_headers(), timeout=15)
        if r.status_code == 200:
...
# ═══════════════════════════════════════════════════════════════
# IP HUNTER ENGINE — REVISED & OPTIMIZED FOR FLAMEPROXIES
# ═══════════════════════════════════════════════════════════════
...
# --- Instruksi 1: Pemetaan Skema & Port ---
def _port_for_scheme(settings: dict, scheme: str) -> int:
    """Mengembalikan port 1080 jika SOCKS5/SOCKS5H, dan 8989 jika HTTP."""
    s = scheme.lower()
...
# --- Instruksi 1 & 4: Pembuatan URL Proxy & Optimasi Geo ---
def _build_proxy_url(settings: dict, new_session: bool = False, city: str = None, state: str = None, candidate: dict = None) -> str | tuple:
    ...
    # Tentukan skema dan target parameter dari candidate (jika probe) atau settings
    ...
    # Siapkan parameter geo & session
    params = f"-country-{country}"

    # Hanya tambahkan state/city jika dikonfigurasi aktif (Instruksi 4)
    ...
    # Tentukan penempatan parameter: di User atau Password
    if target == "user":
...
# --- Instruksi 3: Smart Check Candidates ---
def _proxy_variant_candidates(settings: dict) -> list[dict]:
    """Menghasilkan kombinasi kandidat skema dan target parameter untuk auto-probe."""
...
def _ip_check_one(proxy_url: str, timeout: int = 8, settings: dict = None) -> dict:
    """Melakukan eksekusi validasi IP melalui proxy_url."""
    try:
        import socks
    except ImportError:
        return {"error": "SOCKS support belum terpasang. Jalankan: pip install pysocks requests[socks]", "retryable": False}
    ...
    try:
        r = sess.get(f"http://ip-api.com/json/?fields={fields}", proxies=proxies, timeout=timeout, headers=headers)
        if r.status_code == 200:
            data = r.json() or {}
            if data.get("status") == "success":
                country = data.get("countryCode", "?")
                if country != "BR":
                    return {"error": f"Not Brazil ({country})", "ip": data.get("query")}
    ...
    return {"error": "Proxy tidak merespon (semua endpoint gagal)"}


# --- Instruksi 3: Smart Check Auto-Detect ---
def _ip_check_smart(settings: dict, timeout: int = 15) -> dict:
    """Iterasi kandidat varian proksi. Menyimpan konfigurasi varian yang sukses."""
    candidates = _proxy_variant_candidates(settings)
    last_res = None
    
    for cand in candidates:
        proxy_url, sess_id = _build_proxy_url(settings, new_session=True, candidate=cand)
        if not proxy_url:
            continue
            
        res = _ip_check_one(proxy_url, timeout=timeout, settings=settings)
        if res and "ip" in res and not res.get("error"):
            # Simpan varian sukses ke settings agar pengujian berikutnya langsung lancar
            settings["proxy_protocol"] = cand["scheme"]
            settings["proxy_param_target"] = cand["target"]
            save_settings(settings)
            res["sessid"] = sess_id
            res["proxy_url"] = proxy_url
            res["proxy_scheme"] = cand["scheme"]
            return res
        last_res = res
        
    return last_res or {"error": "Semua kandidat varian proksi gagal terhubung."}


# --- Instruksi 5 & 6: Deduplikasi & Dinamisasi Host Rotator ---
def _ip_scan_sync(settings: dict, target: int = 3, max_attempts: int = 30, min_score: int = 70, timeout: int = 8):
    import queue
    
    # 1. Jalankan probe pintar untuk menemukan varian terbaik
    probe_res = _ip_check_smart(settings, timeout=timeout)
    ...
    # Hasil probe awal langsung dipakai sebagai entri pertama
    if probe_res and "ip" in probe_res and not probe_res.get("error"):
        clean_ips.append(probe_res)
        all_results.append(probe_res)
        seed_key = probe_res.get("proxy_url") or probe_res.get("sessid") or probe_res["ip"]
        seen.add(seed_key)
        lines.append(f"🏆 Clean IP #1: `{probe_res['ip']}` ({probe_res.get('city')})")
    ...
    def _worker_task(attempt_num):
        ...
        res = _ip_check_one(p_url, timeout=timeout, settings=settings)
        if res and "ip" in res:
            res["sessid"] = sess_id
            res["proxy_url"] = p_url
            res["proxy_scheme"] = settings.get("proxy_protocol", "socks5")
    ...
    while len(clean_ips) < target and (time.time() - start_time) < 60:
        try:
            res = results_queue.get(timeout=0.5)
            if not res or "error" in res or not res.get("ip"):
                continue

            ip = res["ip"]
            res_key = res.get("proxy_url") or res.get("sessid") or ip
            if res_key in seen:
                continue
            seen.add(res_key)
...
def _normalize_proxy_for_rotator(proxy_url: str) -> str:
    """Normalisasi URL proksi agar scheme dan port cocok untuk file rotator."""
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "menu_home":
            ...
            await query.edit_message_text(
                f"🌐 *IP Hunter*\n\n🎯 Target Jaringan: *Vivo & Partner MVNO*\n",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
            ...
            s = get_settings()
            await query.edit_message_text("⏳ Mengecek IP saat ini (Smart Probe)...", parse_mode="Markdown")
            loop = asyncio.get_running_loop()
            result = await loop.run_in_executor(None, _ip_check_smart, s, 20)
            ...
            s = get_settings()
            await query.edit_message_text(f"⏳ Sedang mencari {target_count} Clean IP...", parse_mode="Markdown")
            
            loop = asyncio.get_running_loop()
            clean_ips, _, _ = await loop.run_in_executor(None, _ip_scan_sync, s, target_count, 150, 70, 8)
            ...
            for idx, ip_data in enumerate(clean_ips, start=1):
                p_url = ip_data.get("proxy_url")
                if p_url:
                    proxy_urls.append(p_url)
                cards.append(_format_ip_card(ip_data, idx, settings=s))

            if not proxy_urls:
                await query.edit_message_text(
                    "❌ Proxy ditemukan, tetapi URL rotator gagal dibentuk.",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Menu Utama", callback_data="menu_home")]])
                )
                return

            rotator_code = _render_proxy_rotator_file(proxy_urls)
            summary = "\n\n".join(cards[:min(len(cards), 3)])
            if len(cards) > 3:
                summary += f"\n\n… dan {len(cards) - 3} proxy lain sudah dimasukkan ke file rotator."

            await query.edit_message_text(
                f"✅ *Ditemukan {len(proxy_urls)} proxy clean.*\n\n{summary}\n\n📎 File `proxy_rotator.py` dikirim di bawah ini.",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Menu Utama", callback_data="menu_home")]])
            )

            bio = BufferedInputFile(rotator_code.encode("utf-8"), filename="proxy_rotator.py")
            await query.message.reply_document(
                document=bio,
                filename="proxy_rotator.py",
                caption=f"Hasil scan {len(proxy_urls)} proxy clean.",
            )
            ...
            s = get_settings()
            await query.edit_message_text("🧪 Testing proxy connection (Smart Probe)...", parse_mode="Markdown")
            loop = asyncio.get_running_loop()
            result = await loop.run_in_executor(None, _ip_check_smart, s, 15)
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📖 *Daftar Command:* `/start`, `/session`, `/status`, `/settings`", parse_mode="Markdown", reply_markup=home_menu_keyboard())


# --- Instruksi 2: Perbaikan Scope Input Kredensial Proksi ---
@check_auth
async def handle_preset_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Logika Parsing Kredensial Proksi Dibatasi Penuh di dalam Scope ini
    ...
    # Penanganan preset editing biasa
