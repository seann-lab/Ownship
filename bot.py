...
def _build_proxy_url(settings, new_session=False, city=None, state=None):
    ...
    raw_user = settings.get("proxy_user", "")  # 4 spasi
    pw = settings.get("proxy_pass", "")        # 4 spasi
    host = settings.get("proxy_host", "proxy.flameproxies.com")  # 4 spasi
    port = settings.get("proxy_port", 8989)   # 4 spasi
    proto = settings.get("proxy_protocol", "http")  # 4 spasi
    
    if not raw_user or not pw:                  # 4 spasi
        return None                             # 8 spasi
    
    full_pass = pw                              # 4 spasi
    country = settings.get("ip_hunter_country", "br").lower()  # 4 spasi
    full_pass += f"-country-{country}"          # 4 spasi
    
    if state:                                   # 4 spasi
        full_pass += f"-state-{state.lower()}"  # 8 spasi
    
    if city:                                    # 4 spasi
        full_pass += f"-city-{city.lower().replace(' ', '_')}"  # 8 spasi
    
    sess_id = ""                                # 4 spasi
    sess_ttl = settings.get("proxy_session_ttl", 30)  # 4 spasi
    if new_session:                             # 4 spasi ← LINE 2530 HARUS INI
        sess_id = uuid.uuid4().hex[:12]         # 8 spasi
        full_pass += f"-session-{sess_id}-ttl-{sess_ttl}"  # 8 spasi
    
    scheme = "socks5h" if proto == "socks5" else proto  # 4 spasi
    url = f"{scheme}://{raw_user}:{full_pass}@{host}:{port}"  # 4 spasi
    
    if new_session:                             # 4 spasi
        return url, sess_id                     # 8 spasi
    return url                                  # 4 spasi
