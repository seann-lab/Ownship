...
def _ip_scan_sync(settings: dict, target: int = 3, max_attempts: int = 150, min_score: int = 70, timeout: int = 10):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        max_wait = 90 if target > 3 else 60
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "menu_home":
            ...
            proto = s.get("proxy_protocol", "socks5")
            scheme = "socks5" if proto in ("socks5", "socks5h") else "http"
            port = _port_for_scheme(s, proto)
